...
#Assumption: Users that rate more often, e.g. n >=75, have more experience and hence a better judgment which will reduce RMSE.
edx <- edx %>% group_by(userId) %>% filter(n() >= 75) %>% ungroup()
dim(edx)
