...
head(mod)
```
Since we do not know yet which model will perform best on our data set, we will select some popular models and evaluate them in a later step.

Our selected set of models [^3] contains:
* **adaboost:** Adaptive Boosting
* **bayesglm:** Bayes Generalized Linear Model
* **knn:** K-Nearest Neighbors
* **naive_bayes:** Naive Bayes
* **nnet:** Neural Net
* **Rborist:** Extensible, Parallelizable Implementation of Random Forest
* **rf:** Random Forest
...
[^3]: Explaining each model in detail would go beyond the scope of this report. Hence, the models will only be mentioned listed here.
...
Following that, the models are fitted and predictions for the target variable *Attrition_Flag* will be made subsequently with each model using the *test set*.
...
To put the theory into practice, we will compute the *Accuracy* for the selected models first.
...
Now the ensemble model can be evaluated too with the same metrics as before. We compute the *Accuracy* first, and afterwards the *F-measure*.
...
The ensemble model performs relatively good, however with an *Accuracy* of **0.9343534** and an *F-measure* of **0.7637655** it is unfortunately not better than the *AdaBoost* model alone. Hence, we will choose the *AdaBoost* model as our final model.

We can now take a closer look at the *AdaBoost* model to see if we can optimize it further and what else we can find out. To optimize the model, we will use a **tuning grid**. However, we should avoid optimizing it too much since this would most probably result in overfitting the model, and depending on the hardware, would take a very long time. The original optimization (see corresponding R script) was performed with a tuning grid from 0 to 1000 in increments of 50 which took extremely long (~15-20 hours). One could also apply an even finer parameter optimization, e.g. from 1 to 1000 in increments of 1 steps. But this would take even longer and would most probably result in overfitting. Hence, we shall refrain from that. From my personal trials during the model development, I found that the optimal parameters for the *AdaBoost* model at hand are **nIter = 450** with **method = "Adaboost.M1"**. Knowing the optimal parameters, we can massively accelerate the fitting process.
...
Once the optimization is completed, we can inspect the final model parameters. From this, we can see that the *AdaBoost* model uses 450 trees to decide whether a customer is likely to churn or not.

```{r, message = FALSE}
fitAdaboost
```

Now, wen can quickly check how the optimized *AdaBoost* model performs in terms of *Accuracy* and *F-measure*. Therefore we compute the same metrics as before.
